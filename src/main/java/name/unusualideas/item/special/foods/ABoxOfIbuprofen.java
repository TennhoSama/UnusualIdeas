...
public class ABoxOfIbuprofen extends Item {
    private static final int MAX_USE_TIME = 16;

    public ABoxOfIbuprofen(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        if (!world.isClient) {
            ...
            if (IND_R == 0 && ind_r == 0 && s_r > 0.15) {
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.WEAKNESS, 50));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.INSTANT_DAMAGE, 20));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.HEALTH_BOOST, 950));
            }
            ...
            //s_r表示在皮肤过敏可能下的休克可能
            if (IND_R == 0 && ind_r == 0 && s_r <= 0.15) {
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.WEAKNESS, 300));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 250));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 175));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, 300));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.HEALTH_BOOST, 550));
            }
            if (IND_R == 1 && ind_r <= 4 && s_r <= 0.15) {
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.WEAKNESS, 300));
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 250));
            ...
            if (NAUSEA_R == 0 & nausea_r > 4 & IND_R == 0 & ind_r > 0 & s_r > 0.15 || NAUSEA_R == 1 & nausea_r > 14 & IND_R == 1 & ind_r > 4 & s_r > 0.15) {
                user.addStatusEffect(new StatusEffectInstance(StatusEffects.HEALTH_BOOST, 2000));
            }
        }
